...
IntArray::IntArray(std::initializer_list<int> list) :
    IntArray(static_cast<int>(
        list.size())) // use delegating constructor to setup initial array
{
    // initialize our array with the list
    int count{0};
...
{
    // if new length is not the same as old length, we can reallocate memory
    ...
    for (int count{0}; auto element : list)
    {
        m_data[count] = element;
        ++count;
    }
